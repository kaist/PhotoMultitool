...
class Keyboard(HumanInterfaceDevice):
    HID_INPUT_REPORT = bytes((
        ...
        # Consumer Control (Media keys) - ДОБАВЛЕНО
    ...
    def __init__(self, name="Bluetooth Keyboard"):
        ...
        self._media_buf = bytearray(2)  # Буфер для медиа-клавиш
        self._modifiers = 0
    ...
    def send_media_key(self, keycode):
        """Отправка медиа-клавиши (Consumer Control)"""
        if self.is_connected() and self.h_rep is not None:
            # Устанавливаем код медиа-клавиши
            self._media_buf[0] = keycode & 0xFF
            self._media_buf[1] = (keycode >> 8) & 0xFF
            
            # Отправляем с Report ID = 2 (для медиа-клавиш)
            media_report = bytes([0x02]) + self._media_buf
            self._ble.gatts_write(self.h_rep, media_report)
            self._ble.gatts_notify(self.conn_handle, self.h_rep, media_report)
            
            # Сразу отпускаем клавишу
            time.sleep_ms(50)
            self._media_buf[0] = 0
            self._media_buf[1] = 0
            media_report = bytes([0x02]) + self._media_buf
...
class App:
    ...
    def left(self):
        print('media')
        self.dev._tap(const(0x80))
        
        self.dev.keyboard.send_media_key(const(0x80))
    ...
    def start(self, app):
        ...
        self.app.callback_table['right']=self.left
